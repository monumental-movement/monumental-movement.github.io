...
<div class="mermaid">
graph TD
    Moog[Moog Modular] --> TechnoOrigins[early detroit techno]
    ARP[ARP Synth] --> TechnoOrigins
    TechnoOrigins --> ModularTechno[The foundation of Modular Techno]
</div>
...
<div class="mermaid">
graph LR
    Berlin --> Berghain[Berghain Club]
    Berlin --> Sisyphos[Sisyphos Club]
    Artist[Surgeon, ØPhase, SNTS] --> Berlin
    Artist --> Online[YouTube/Forum]
</div>
...
A typical configuration of a live set would look like this:
...
graph LR
    Clock --> Seq[sequencer]
    Seq --> VCO[VCO / sound source]
    VCO --> VCF[filter]
    VCF --> VCA[VCA/Volume control]
    VCA --> Mixer[mixer]
    Mixer --> FX[effect module]
    FX --> Out[final output]
</div>
...
<div class="mermaid">
graph LR
    Berlin[Berlin] --> Clubs[club scene]
    Berlin --> Superbooth[Superbooth event]
    Clubs --> ModularTechno[Modular TechnoCulture]
...
This adds an element of ""shaking.''
...
<div class="mermaid">
flowchart TD
    A[fixed loop] -->|contrast| B[Stochastic/variable sequences]
    B --> C[organic groove]
    C --> D[Features of Modular Techno]
</div>
...
<div class="mermaid">
graph LR
    Digital[digitization] --> Future[Evolution of Modular Techno]
    Hybrid[hybrid live] --> Future
    Community[community culture] --> Future
</div>
...
Below is an example visualizing the patch structure of a typical Modular Techno liveset.
This is a patch diagram constructed with 4 layers: rhythm, baseline, noise, and FX.

### Patch example 1: Kick + baseline generation patch
...
### Patch example 2: Rhythm generation with random elements
...
### Patch example 3: Noise + industrial texture generation
