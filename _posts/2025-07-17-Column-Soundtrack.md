...
<div class="mermaid">
flowchart TD
  title 映画音楽の主要メディア：記録と再生の技術的変遷

  subgraph A["🎞 フィルム自体への録音（サウンド・オン・フィルム）"]
    A1["光学録音方式（Optical Soundtrack）\n1930〜1950年代に一般化\n→ 画面脇に波形画像として音を記録し、投影時に再生"]
    A2["磁気録音方式（Magnetic Soundtrack）\n1950〜60年代登場\n→ ステレオ録音・ダイナミックレンジの向上"]
    A1 --> A2
  end

  subgraph B["💿 レコード（Vinyl LP / EP）"]
    B1["1950〜80年代に隆盛\n映画音楽の一般流通メディア\n→ ハイライト・抜粋盤が主流"]
  end

  subgraph C["📼 カセットテープ / 8トラック"]
    C1["1970〜80年代普及\n→ カーオーディオやポータブル機器で映画音楽を携帯"]
  end

  subgraph D["💽 CD（コンパクトディスク）"]
    D1["1980年代以降普及\n高音質・長時間・曲順の自由度が高い"]
    D2["フルスコア収録が可能となり映画音楽鑑賞の黄金時代へ"]
    D3["専門レーベル登場：La-La Land / Intrada / Varese Sarabande"]
    D1 --> D2 --> D3
  end

  subgraph E["📀 DVD / Blu-ray：映像＋音楽"]
    E1["サウンドトラックを“再体験”するメディア"]
    E2["5.1ch / DTSなど立体音響技術による空間表現の進化"]
    E1 --> E2
  end

  subgraph F["💻 デジタル音源（MP3 / WAV / FLAC / AAC）"]
    F1["iTunes / Amazon MP3などで音源販売"]
    F2["Bandcampなどで独立系作家が発信"]
    F3["フルスコアから未発表曲まで高解像度で配信可能"]
    F1 --> F2 --> F3
  end

  subgraph G["☁ サブスクリプション（Spotify / Apple Musicなど）"]
    G1["“サウンドトラックも聴き流す時代”へ"]
    G2["アルバム単位ではなくプレイリスト化"]
    G3["映画音楽の“機能性”と“芸術性”の境界が再び問われる"]
    G1 --> G2 --> G3
  end
