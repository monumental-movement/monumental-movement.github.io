...
节奏编程并不是精确排列点的问题。這是一種設計行為：如何劃分時間，在哪裡強調，在哪裡留白。如果效能是一系列瞬時決策，那麼程式設計就類似於從整個時間的鳥瞰角度來思考。
...
遵循网格的安排完美地固定了时间，但不移动它。這是故意引入偏差的地方。
...
即使是同樣的120 BPM，如果強調backbeat，也會感覺更快，如果拉寬音程，就會感覺更慢。
