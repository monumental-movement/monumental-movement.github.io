...
리듬 프로그래밍은 타점을 정확하게 정렬하는 작업이 아닙니다. 시간을 어떻게 나누고, 어느 위치를 강조하고, 어디에 여백을 남기는가 하는 설계 행위이다. 연주가 순간적 판단의 연속이라고 하면, 프로그래밍은 시간 전체를 부감하는 사고에 가깝다.
...
그리드를 완전히 따르는 배열은 시간을 고정하지만 움직이지 않습니다. 거기서 도입되는 것이 의도적인 어긋남이다.
