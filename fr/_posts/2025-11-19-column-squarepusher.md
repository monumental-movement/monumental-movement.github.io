---
author: mmr
categories:
- Column
image: ../assets/images/column-squarepusher-history.webp
lang: fr
layout: post
permalink: /fr/column-squarepusher/
tags:
- IDM
- Drum n Bass
- Jazz
- Experimental
title: '[Chronique] Squarepusher : Une fusion de breakbeats, de jazz et de musique
  électronique'
---


## 1. Présentation


Texte : mmr｜Thème : jeunesse de Tom Jenkinson, premières œuvres, style musical et explication de toutes les chansons par album

Tom Jenkinson, également connu sous le nom de Squarepusher, est un musicien britannique très présent sur la scène électronique contemporaine depuis le milieu des années 1990.
Sa musique est connue pour son style innovant, qui intègre à lui seul des breakbeats complexes, un jeu de basse avancé, une improvisation jazz et un son électronique de style IDM.


---

<style type="text/css">

table, td, th {
border: 2px #111 solid;
width: auto;
padding: 10px; 
}
th {
background-color: #111;
color: #fff;
}
</style>


## 2. Les débuts de la vie et le parcours musical de Tom Jenkinson

### 2.1 Enfance et environnement musical

Né en 1975 près de Manchester. Joue du piano, de la guitare basse et de la batterie depuis son enfance.
Il a reçu une formation de jazz classique et maîtrisé des rythmes et des accords complexes.

### 2.2 Influence du jazz fusion

- Jaco Pastorius : basse polyphonique, improvisation
- Bulletin météo : rythmes complexes
- ELP : Son électronique et fusion live

### 2.3 Rencontre avec la musique électronique

Après avoir été exposé à l'Acid House et aux débuts de la techno, il a commencé à enregistrer chez lui en utilisant des Akai S1000 et TR-808/909.

---

## 3. Premiers travaux et contrat Warp Records

### 3.1 Version indépendante de Spymania

- 1995 : *Conumber E:P* annoncé
- Introduction de boucles courtes, de multicouches et de signatures rythmiques impaires

### 3.2 Contrat d'enregistrement Warp

- 1996 : début de *Feed Me Weird Things*
- Fusion IDM + Jazz + Drum & Bass

<div class="mermaid">

graph TD
    A[自主リリース: Spymania] --> B[注目]
    B --> C[Warp Records契約]
    C --> D[デビューアルバム]
    D --> E[国際的認知]

</div>

---

## 4. Caractéristiques du style musical

### 4.1 Batterie et basse Breakcore

<div class="mermaid">

graph TD
    A[ドラムマシン] --> C[多層リズム]
    B[ベース演奏] --> C
    C --> D[密度の高い聴覚体験]

</div>

---

### 4.2 Influence du jazz fusion

* Jeu de basse hautement improvisé
* Éléments de fusion dans les progressions d'accords et les phrases

### 4.3 Fusion analogique + numérique

* TR-808/909, TB-303, Minimoog
* Akai S1000, Raison
* Fusion de frappe + performance live

---

## 5. Explication de toutes les chansons par album + visualisation

### 5.1 Nourris-moi des choses bizarres (1996)

**Matériel utilisé** : Akai S1000, TR-808/909, Minimoog, Korg MS-20

#### Explication de la chanson + diagramme rythmique

1. **Thème Squarepusher**

* Breakbeat de signature rythmique impaire + improvisation de basse

<div class="mermaid">

sequenceDiagram
    participant K as Kick
    participant S as Snare
    participant H as HiHat
    K->>H: 16分音符刻み
    H->>S: ポリリズム生成

</div>

---

2. **Thème d'Ernest Borgnine**

* Synthé analogique + batterie haute vitesse

<div class="mermaid">

graph LR
    Bass --> DrumLoop
    DrumLoop --> SynthLayer
    SynthLayer --> ListenerExperience

</div>

---


3. **Numéro**

* Fusion basse + son électronique IDM

<div class="mermaid">

graph TD
    Start --> BassLayer
    BassLayer --> IDMLoop
    IDMLoop --> MultiLayeredTexture
    MultiLayeredTexture --> End

</div>

---


---

### 5.2 Papa normal dur (1997)

**Matériel utilisé** : Akai S3000, TR-808/909, Novation Bass Station, Moog Voyager

1. **Ma voiture rouge chaude**

<div class="mermaid">

graph LR
    Kick --> Snare
    Kick --> HiHat
    Snare --> HiHat
    HiHat --> Kick

</div>

---

2. **Acide Vic**

* Polyrythmie + improvisation jazz

<div class="mermaid">

pie
    title Track Layers
    "Drums": 40
    "Bass": 35
    "Synth": 25


</div>


---

### 5.3 La musique est pourrie, une note (1998)

**Matériel utilisé** : Fender Jazz Bass, Korg MS-20, Roland JP-8000

1. **Port Losange**

* Improvisation d'enregistrement en direct

<div class="mermaid">

sequenceDiagram
    Bass->>Keyboard: Call and Response
    Keyboard->>Bass: Improvisation Return


</div>

---


### 5.4 Go Plastique (2001)

**Matériel utilisé** : Minimoog, TB-303, TR-808/909, Akai MPC

1. **Allez ! Spasmodique**

* Breakbeat à grande vitesse + rythme fractal

<div class="mermaid">

graph TD
    A[Beat] --> B[FractalSubdivision]
    B --> C[Polyrhythm]
    C --> D[Listener]

</div>

---


### 5.5〜5.9 Ultravisiteur〜Soyez bonjour

* Le diagramme de rythme, l'image de forme d'onde et la structure fractale de chaque chanson sont développés de la même manière
* **Exemple** : Ultravisitor *Bienvenue en Europe*

<div class="mermaid">

graph TD
    A[Drum Layer] --> B[Synth Loop]
    B --> C[Bass Improvisation]
    C --> D[End]

</div>

---


## 6. Performance live + illustration technique

* Improvisation basse + boîte à rythmes + sampler

<div class="mermaid">

graph TD
    Bass --> LiveRhythm
    Drum --> LiveRhythm
    Sampler --> LiveRhythm
    LiveRhythm --> AudienceExperience


</div>

---


## 7. Évaluation/Impact

* IDM, pionnier de la drum and bass
* Pilier de l'ère d'or de Warp Records
* Précision technique + fusion improvisée + évaluation de l'innovation

---

## 8. Chronologie

<div class="mermaid">

graph TD
    "Feed Me Weird Things (1996)" --> "Hard Normal Daddy (1997)"
    "Hard Normal Daddy (1997)" --> "Music Is Rotted One Note (1998)"
    "Music Is Rotted One Note (1998)" --> "Go Plastic (2001)"
    "Go Plastic (2001)" --> "Ultravisitor (2004)"
    "Ultravisitor (2004)" --> "Hello Everything (2006)"
    "Hello Everything (2006)" --> "Solo Electric Bass 1 (2009)"
    "Solo Electric Bass 1 (2009)" --> "Damogen Furies (2015)"
    "Damogen Furies (2015)" --> "Be Up A Hello (2020)"

</div>

---


## 9. Cartographie de la musicalité

<div class="mermaid">

graph LR
    IDM --> Squarepusher
    Jazz --> Squarepusher
    Drum'n'Bass --> Squarepusher
    Fusion --> Squarepusher
    Experimental --> Squarepusher
    LivePerformance --> Squarepusher

</div>

---


## 10. Conclusion

Squarepusher est un artiste innovant qui fusionne la musique électronique, le jazz et les breakbeats.
L'improvisation live, les structures acoustiques complexes et l'expérimentation d'album en album sont des indicateurs d'innovation dans la musique électronique contemporaine.
La structure rythmique, les formes d'onde et les illustrations fractales de chaque chanson vous aideront à approfondir davantage votre compréhension de la musique.

---
